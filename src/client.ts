...
export class WebSocketClient<Method extends string = string, Notice extends string = string, ListeningMethod extends string = string> extends BaseWsClient<WebSocket, Method, Notice, ListeningMethod> {
	...
	async open() {
		await new Promise((resolve, reject) => {
			...
			this.webSocket.on('close', async () => await this.closeHandler());
